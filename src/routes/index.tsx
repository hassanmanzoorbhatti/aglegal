...
import heroImage from "../assets/ag-hero.jpg";
import practicalSupportImageOne from "../assets/ag-practical-support-1.jpg";
import practicalSupportImageTwo from "../assets/ag-practical-support-2.jpg";
...
function DetailFeature() {
  return <section className="grid grid-cols-1 border-t border-clause-border md:grid-cols-2"><div className="grid grid-cols-2"><RevealImage src={practicalSupportImageOne} alt="Dispute support and legal correspondence" wrapperClassName="h-full" className="h-full min-h-[360px] w-full object-cover"/><RevealImage src={practicalSupportImageTwo} alt="Contract drafting and review" wrapperClassName="h-full border-l border-clause-border" className="h-full min-h-[360px] w-full object-cover"/></div><div className="px-4 py-14 md:border-l md:border-clause-border md:px-14 md:py-20"><Reveal><Eyebrow>Practical support</Eyebrow></Reveal><AnimatedText as="h2" text="Get your position set out clearly and professionally." className="mt-5 font-serif-display font-normal leading-[1.15] text-clause-heading text-[clamp(1.625rem,1rem+2.5vw,2.5rem)]"/><Reveal delay={0.2}><p className="mt-6 max-w-[520px] font-grotesk text-[15px] leading-[1.75] text-clause-muted md:text-[16px]">From pre-action and complaint letters to contract reviews, service agreements and terms & conditions, AG Legal Group focuses on well-structured written work that helps clients understand and strengthen their position.</p></Reveal><Reveal delay={0.28}><div className="mt-8"><TextLink to="/services">View service list</TextLink></div></Reveal></div></section>;
}

function Index() {
  return <div className="min-h-screen bg-clause-bg px-2 pb-2 md:p-0"><div className="mx-auto min-h-[calc(100vh-0.5rem)] max-w-[1320px] border border-clause-border md:min-h-screen md:border-x md:border-y-0"><Navbar/><div className="h-16 lg:hidden" aria-hidden="true"/><Hero/><HeroStats/><Ribbon className="h-8 border-t border-clause-border"/><Introduction/><Services/><DetailFeature/><Approach/><LeadershipBlock/><FinalCta/><Footer/></div><ScrollDot/></div>;
}
